...
#
# ⚠️ **母集団の異質性を捨ててはいけない。** 「同じ水準の合成打者9人 vs 合成投手1人」で組むと、
# softmax が非線形なので**平均的な対戦の結果 ≠ 母集団平均の結果**になり、
# δ=0 セルが K% 11.3% (実リーグ 19.4%) / BABIP .353 と大きく外れる。実在の打線・投手陣を
...
# **そのままこのグリッドの軸として読める**。
const BATTER_LEVEL_KEYS: Array[String] = PSReferencePopulation.BATTER_LEVEL_KEYS
const PITCHER_LEVEL_KEYS: Array[String] = PSReferencePopulation.PITCHER_LEVEL_KEYS

# 母集団の抽出は PSReferencePopulation (run_platoon_probe と共有) が持つ。
# ⚠️ 妥当性の確認は reference_* の health checks で行う。
const POPULATION_PITCHERS_PER_TEAM: int = PSReferencePopulation.DEFAULT_PITCHERS_PER_TEAM

# --- 較正ターゲット ---
# ⚠️ **一軍とファームのどちらが高いかは問わない。** 大きく逸脱していなければ十分なので、
# レベル差の帯は**両側**で見る (符号は固定しない)。
# 見なければならないのは **専用球団の勝率** = 非対称マッチアップの感度。
...
# --- 一軍相当プロファイルの実測 ---
# 抽出条件は PSReferencePopulation が単一ソース (run_platoon_probe と共有)。
func _build_reference_population() -> Dictionary:
	return PSReferencePopulation.build(POPULATION_PITCHERS_PER_TEAM)
...
func _health_checks(anchors: Dictionary, reference_cell: Variant) -> Array:
	...
	# 根拠の無い仮定に依存するため、pass/fail は付けず観測値としてだけ出す。
