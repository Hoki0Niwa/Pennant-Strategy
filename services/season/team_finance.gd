...
#  - 育成選手 (development_player == true) は NPB 同様に**人数上限なし**。
#    抱え込みは枠でも目標人数でもなく
...
# 来季開幕時の支配下目標。オフの編成は「この人数へ寄せる」計画ベースで動く:
...
# --- 固定予算 ---
# **全球団同額・毎年不変**。順位ボーナス等で毎オフ再計算する方式は採らない (年を追うごとに
# 予算が変わり続け、補強AIの閾値が意味を失うため)。単位は万円で、値は年俸総額が最も高い球団
# (約42.09億) が超過扱いにならないよう 43億に切り上げてある。
# **この定数が予算の単一ソース**。team.funds は初期シード CSV / セーブ / SQLite のどこから来ても
# `apply_fixed_budget` で必ずこの値へ揃える。シード CSV の funds 列は進化後ワールドを
