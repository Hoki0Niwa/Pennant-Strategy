...
#
# ⚠️ **ただし実クラブの元NPB比率は 21〜24%** (ハヤテ 8/38・オイシックス 11/45)
...
# 投手/野手は各1人まで。無制限にすると評価上位4人が全員野手になり、主な弱点である投手層へ
# 届かない。総枠を増やすための制約ではなく、2枠を片側で使い切らないための制約。
...
#
# ⚠️ **在籍は指名の有無と無関係**で、年齢の扱いも供給元で分けない。NPB 未経験者にだけ
#    「27歳から整理・32歳で必ず退団」のような強制退団は掛けない。
...
#
# ⚠️ **表示能力の固定値 (center / max_display / variance) で水準を決めてはいけない。**
#    固定値は母集団が変われば意味を失い、較正が孤児になる
...
# (world seed に依存させると farm report の2回実行で 中止16件→0件・BB% .104→.145 と揺れる)。
