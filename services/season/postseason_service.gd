...
static func _simulate_series(season: PSSeason, series: Dictionary) -> Dictionary:
	...
	while top_wins < win_target and challenger_wins < win_target:
		...
		var game_outcome: Dictionary = _simulate_one_postseason_game(season, away_id, home_id)
...
static func _simulate_one_postseason_game(season: PSSeason, away_id: int, home_id: int) -> Dictionary:
	...
	return PSGameLoop.simulate_game(away_setup, home_setup)
