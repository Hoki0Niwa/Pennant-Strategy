...
# ⚠️ 閾値がかかる相手は **starter_shape_advantage** (能力水準を打ち消した差) で、生の
# starter_score ではない。スケールが違うので、値は `tools/run_farm_club_diag --roles` の
# 分位点から取り直すこと (+0.3 まで上げると全水準で先発 ~28% まで落ちる)。
...
#
# 補正しないと、質 z 平均 -0.85 のファーム専用球団は **22人中0人**が先発判定になり、
# 二軍戦がブルペンデー連発 (先発した投手 19-20人 / 完投31) になる。
# 一方 質 z 平均 +1.3 の 12球団は 40人中 15-17人が先発になる。
#
