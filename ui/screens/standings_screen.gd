...
func _draw_table(rect: Rect2, title: String, right_label: String, columns: Array, rows: Array) -> void:
	...
	if not right_label.is_empty():
		_text_right(right_label, rect.end.x - 18, rect.position.y + 30, 12, MUTED, 180)
...
func _draw_table_row(rect: Rect2, inner_x: float, factor: float, columns: Array, row: Dictionary, ry: float, row_h: float) -> void:
	...
	for col_value in columns:
		...
		match fmt:
			...
			"str":
				_text(str(row.get(key, "")), Vector2(cx + 4.0, ty), 12, MUTED if not is_self else base_color, w - 6.0)
			"diff":
