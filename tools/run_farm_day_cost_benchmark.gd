...
# Phase 0: 二軍戦を日次並列グループへ相乗りさせたときの限界コスト実測。
#
# 二軍ロスターはまだ存在しないので、代わりに **影の世界** (14球団ぶんの、選手もレコードも
# 完全に独立した複製) を作って「1日に何試合まで同時計算できるか」を測る。実在の試合を
# 使い回すと同じ PSPlayerSeasonRecord へ複数ワーカーが書き込んでレースになるため、
# 独立した複製であることが測定の前提。試合内容の妥当性は問わない。
#
# モード:
#   first_only … 一軍6試合のみ。影の世界を作らない = 現行の挙動そのもの (ベースライン)
#   merged     … 一軍6 + 影7 = 13試合を 1 つの WorkerThreadPool グループで計算 (提案する設計)
#   split      … 一軍6 と 影7 を別グループで順に計算 (素朴な代替案)
#
